...
test.describe('Environment Settings UI', () => {
  ...
  test('should update and save the default deploy pull policy in Docker settings', async ({ page }) => {
    await openLocalEnvironment(page);
    ...
    try {
      ...
      if (!page.isClosed()) {
        await page
          ...
          .first()
          .click();
        const currentValue = ((await page.locator('#defaultDeployPullPolicy').textContent()) || '').trim();
